    ...
    for (let i = 0; i < this.particles.length; i++) {
      ...
      if (this.mouse.active) {
        ...
        if (mouseDist < this.mouse.radius) {
          ...
          this.ctx.strokeStyle = colors.mouseWeb.replace(/[\d.]+\)$/, `${alpha})`);
...
const CreatorFlow = {
  ...
  init() {
    ...
    const syncInputs = ['crush-name', 'sender-name', 'movie-title', 'invite-message', 'date-val', 'time-val', 'location-val'];
    syncInputs.forEach((id) => {
      document.getElementById(id).addEventListener('input', () => this.syncPreview());
    });
  ...
  syncPreview() {
    ...
    const movie = document.getElementById('movie-title').value || "Spider-Man: Brand New Day";
    const msg = document.getElementById('invite-message').value || "Will you swing by the cinema with me?";
  ...
  generatePortalLink() {
    ...
    const theme = document.querySelector('input[name="theme"]:checked').value;
    const behavior = document.querySelector('input[name="no-behavior"]:checked').value;
    const tbd = document.querySelector('input[name="decision-mode"]:checked')?.value === 'receiver';
    const tgToken = document.getElementById('tg-token')?.value.trim();
    const tgChatId = document.getElementById('tg-chat-id')?.value.trim();
    if (!crush || !sender || !movie || !msg || (!tbd && (!dateVal || !timeVal || !loc))) {
      alert("Please fill all required inputs to stabilize the multiverse portal link!");
      return;
    }
    const payload = {
      c: crush,
    ...
    navigator.clipboard.writeText(generatedUrl).then(() => {
      const copyBtnText = document.getElementById('btn-copy-link').querySelector('.btn-text');
...
document.getElementById('btn-copy-link').addEventListener('click', () => {
  ...
  navigator.clipboard.writeText(input.value).then(() => {
    const btnText = document.getElementById('btn-copy-link').querySelector('.btn-text');
...
const ProposalReceiver = {
  ...
  init(decodedData) {
    ...
    // Dates formatting
    let dateFormatted = `${this.data.d} @ ${this.data.t}`;
    if (this.data.tbd) {
      dateFormatted = "To Be Decided";
    } else if (this.data.d) {
      try {
        const parts = this.data.d.split('-');
        const dateObj = new Date(parts[0], parts[1] - 1, parts[2]);
        const options = { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' };
        dateFormatted = `${dateObj.toLocaleDateString('en-US', options)} @ ${this.data.t}`;
      } catch (e) {}
    }
    const dateEl = document.getElementById('display-datetime') || document.getElementById('display-date-time');
    ...
    document.getElementById('btn-open-portal').addEventListener('click', () => this.openTransmission());
    
    // YES action bindings
    document.getElementById('btn-yes').addEventListener('click', () => this.triggerYesDate());
    ...
    document.getElementById('btn-fix-timeline').addEventListener('click', () => {
      document.getElementById('canon-event-overlay').classList.add('hidden');
      this.triggerYesDate();
    });
    // Calendar Integration
    document.getElementById('btn-add-calendar').addEventListener('click', () => this.downloadCalendarICS());
    
    // Screenshot / share simulation
    document.getElementById('btn-screenshot').addEventListener('click', () => this.shareConfirmedMessage());
  },
  openTransmission() {
    AudioFX.playGlitch();
  ...
  triggerWebSnapNoButton() {
    ...
    noBtn.style.transition = 'transform 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.2)';
  ...
  triggerYesDate() {
    if (this.data.tbd && !this.data.chosen) {
      ...
      document.getElementById('receiver-choice-view').classList.remove('hidden');
    ...
    document.getElementById('ticket-people').innerText = `${this.data.c} & ${this.data.s}`;
    ...
    if (this.data.th === 'bully') {
      ...
      document.getElementById('success-prompt-text').innerText = "Double lock! Emo dance stabilized. See you there!";
    } else {
      stamp.innerText = "CANON EVENT LOCKED 🔒";
      document.getElementById('success-prompt-text').innerText = "A ticket has been registered in the arachnid database. See you in the theater!";
    }
    ...
    if (this.data.d) {
      try {
        ...
        dateFormatted = dateObj.toLocaleDateString('en-US', options).replace(',', ' @');
      } catch (e) {}
    }
  ...
  sendTelegramNotification() {
    let niceDate = `${this.data.d} @ ${this.data.t}`;
    if (this.data.d) {
      try {
        const parts = this.data.d.split('-');
        const dateObj = new Date(parts[0], parts[1] - 1, parts[2]);
        const options = { weekday: 'long', month: 'short', day: 'numeric' };
        niceDate = `${dateObj.toLocaleDateString('en-US', options)} @ ${this.data.t}`;
      } catch (e) {}
    }
  ...
  spawnConfetti() {
    ...
    for (let i = 0; i < numConfetti; i++) {
      ...
      const animationTimer = setInterval(() => {
        ...
        el.style.left = `${px}px`;
        el.style.top = `${py}px`;
        if (py > window.innerHeight + 20 || px < -30 || px > window.innerWidth + 30) {
...
END:VCALENDAR`;
        ...
        alert("Failed to synthesize ICS calendar file automatically. Please save details manually!");
      }
    }
  },
    ...
    if (navigator.share) {
      navigator.share({
        ...
        navigator.clipboard.writeText(shareText).then(() => {
          alert("Date details copied to clipboard! Send it to them to double lock the canon event!");
        });
      });
    } else {
      navigator.clipboard.writeText(shareText).then(() => {
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (portalCode) {
    ...
    if (decoded) {
      ...
      alert("Multiverse Portal decrypted with key error! Generating portal repair...");
